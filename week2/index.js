...
var l = '8';
// 請文字解釋為什麼
console.log(k * l > 21); // 在 k * l 的運算會進行隱式轉型，將字串 '8' 轉換為數字 8，然後進行乘法運算。 k * l 的結果是 3 * 8 = 24。 24 大於 21，所以比較運算結果為 true。
...
let minimumPerformance = 50000;
if (performanceData.coaches[0].performance < minimumPerformance) {
  performanceData.coaches[0].performance += minimumPerformance - performanceData.coaches[0].performance;
}

// 練習：第二位教練（可將下方程式碼註解移除，完成答題）
if (performanceData.coaches[1].performance < minimumPerformance) {
  performanceData.coaches[1].performance += minimumPerformance - performanceData.coaches[1].performance;
}
...
let calorieBurn = 0;
let treadmillMinute = 30;
let yogaMinute = 40;
let bicycleMinute = 10 + 10;
let treadmillBurn = activities.treadmill.caloriesPerMinute;
let yogaBurn = activities.yoga.caloriesPerMinute;
let bicycleBurn = activities.bicycle.caloriesPerMinute;

calorieBurn = treadmillMinute * treadmillBurn + yogaMinute * yogaBurn + bicycleMinute * bicycleBurn;
// 練習：計算小明今日消耗的卡路里
